...
void loop() 
{
  ...
  timerInterval = map(analogRead(adcPin), 0, 1023, 10, 2000); //map ADC read from 0-1023 to 10-2000
  
  //now that we are no longer using delay(), we don't have to wait a whole second for pot value to update!
  //our timer is NON-BLOCKING, so we can read the pot and update our interval while we wait for our timer to trigger

  if (currentTime - lastTime >= timerInterval)  //if the amount of time that has passed since last event is >= to our timer interval...
  {
    lastTime = currentTime; //store current time as last time (this is when our timer event is triggered)
    ledState = !ledState; //flip LED state
    Serial.println("<><><><>____TIMER TRIGGERED!____<><><><>"); //print that timer is triggered
  }

  digitalWrite(ledPin, ledState); //set LED to current state (ON/OFF)
